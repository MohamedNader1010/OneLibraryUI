...
export class AddEditComponent implements OnInit, OnDestroy {
  ...
  getPrice(serviceTypeId: number, index: number) {
    ...
    return this.subscriptions.push(
      this.servicePriceService
        .getServicePricePerClient(this._clientTypeId, serviceTypeId)
        .subscribe(
          (res) => {
          ...
          (err) =>
            this.toastr.error(
              err,
              "Something went wrong when getting service price."
            )
        )
    );
  }
      ...
      this.prices.forEach((element) => {
        if (element.key == index) {
          ...
          qty
            ...
            : priceInput?.setValue(element.value);
        }
      });
    }
  ...
  getAllNotes() {
    this.subscriptions.push(
      this._note.getAll().subscribe({
        ...
        error: (e) => {
          this.toastr.error(e.message, "تحميل البيانات ");
        },
      })
    );
  }
  ...
  computeDiscountPercent(): number {
    ...
    let percent = ((discountAmount?.value / this.totalPrice) * 100)
      .toFixed(2)
      .toString();
  ...
  computeDiscountAmount(): number {
    ...
    let value = ((discountPercent?.value / 100) * this.totalPrice)
      .toFixed(2)
      .toString();
  ...
  private clearPriceAndQuantityInput(index: number) {
    this.OrderDetails.controls[index].get("price")?.setValue("");
  ...
  getSingle = (id: number) =>
    this.subscriptions.push(
      this._order.getOne(id).subscribe((data) => this.fillFormWithData(data[0]))
    );
  ...
  private validateDiscountAmountAndPercent() {
    ...
    if (discount !== percent) {
      this.toastr.error(
        "discount amount not equals to discount percent",
        "ERROR"
      );
