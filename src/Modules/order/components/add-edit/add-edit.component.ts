...
export class AddEditComponent implements OnInit {
	...
	getSingle = (id: number) => this._order.getOne(id).subscribe((data) => {});
	...
	handleSubmit() {
		if (this.orderForm.valid) {
			if (this.id) {
				this._order.update(this.id, this.orderForm.value).subscribe({
					next: () => {
						this.router.navigate(["services"]);
					},
				});
			} else {
				this._order.add(this.orderForm.value).subscribe({
					next: () => {
						this.router.navigate(["services"]);
					},
				});
